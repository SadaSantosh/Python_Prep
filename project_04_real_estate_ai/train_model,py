...
location_score = np.random.randint(1, 10, n_samples)  # 1 (Poor) to 10 (Prime)

# Price Formula with Noise: Base + Sqft*180 + Beds*15k + Baths*20k - Age*1.2k + Location*25k
price = (
    50000 
    + (sqft * 180) 
    + (bedrooms * 15000) 
...
print(f"✔ R² Score (Accuracy fit): {r2 * 100:.2f}%")
print("="*40)

# 7. Save Model & Scaler Artifacts
joblib.dump(model, 'real_estate_model.pkl')
joblib.dump(scaler, 'real_estate_scaler.pkl')
print("✅ Saved 'real_estate_model.pkl' and 'real_estate_scaler.pkl' successfully!")